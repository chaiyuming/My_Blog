...
try:
    ...
    if UEDITOR_UPLOAD_TO_SERVER:
        # UEDITOR_UPLOAD_PATH = settings.FASTDFS_SERVER_DOMAIN
...
class UploadView(View):
    ...
    #     上传文件到自己的服务器
    #     """
    #     image_ext_name = filename.name.split('.')[-1]
    #     upload_res = FDFS_Client.upload_by_buffer(filename.read(), file_ext_name=image_ext_name)
    #     with open(os.path.join(UEDITOR_UPLOAD_PATH, filename), 'wb') as fp:
    #         for chunk in upfile.chunks():
    #             fp.write(chunk)
    #     url = reverse("ueditor:send_file", kwargs={"filename": filename})
